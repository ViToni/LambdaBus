...
//@formatter:off
/**
 * This interface defines the API of an event bus using class types as implicit
 * topics for which {@link Consumer}s can be subscribed. Events are published by
 * posting them to the bus using the {@link #post(Object)} or
 * {@link #post(Object, ThreadingMode)} method defined in the interface.
 *
 * <blockquote>
 *
 * <pre>
...
// for every "String" event published to the bus call System.out.println() with this
// String as parameter
 ...
 * <p>
 * Another approach is to use a reference to the {@link #post(Object)} method so
 * that publisher can be agnostic about the event bus itself.
 * </p>
 *
 * <blockquote>
 *
 ...
 * <p>
 * Subscription is mainly based on classes but {@link Consumer}s might also
 * subscribe to an interface. Interface lookup will occur only if no subscriber
 * for the class of the event has been found.<br>
 * The event bus will scan the event's class for its directly implemented
 * interfaces, lookup of sub-classes and interfaces of sub-classes is omitted
 ...
 * </blockquote>
 *
 * Subscribers for {@code BaseImpl} will receive only events of type
 * {@code BaseImpl}, events of subclasses such as {@code SubImpl} won't
 * match.<br>
 * Subscriptions for {@code A}, {@code B} or {@code C} will only receive events
 * of type {@code BaseImpl} if there is no {@link Consumer} subscribed for
 * {@code BaseImpl}.<br>
 * Lookup of subscribers will occur in order of appearance in the class file,
 * e.g. for an event of type {@code BaseImpl} lookup will search in this order
 * for matching subscribers:<br>
 *
 * <blockquote>
 *
 * <pre>
 ...
 * </p>
 *
 * <h2>Dead events - events without subscribers</h2> Events for which no
 * subscriber could be found (neither class nor directly implemented interfaces)
 * will be posted to the bus wrapped in an instance of {@link DeadEvent}.<br>
 * To avoid creation of unused objects a {@link DeadEvent} should be created
 * only if there is a subscriber for the {@link DeadEvent} class.<br>
 * A use case could be to log such events to detect missing subscriptions.
 *
 * <h2>Publishers &amp; Subscribers</h2>
 * <p>
 * The {@link LambdaBus#post(Object)} can be passed as a method reference in
 * form of a {@link Consumer}.<br>
 * Publisher using the {@link Consumer} have no dependency to {@link LambdaBus}
 * implementations.
 * </p>
 *
 * <blockquote>
 ...
 *
 * <p>
 * A subscriber is just a {@link Consumer} associated with a class or interface
 * and does not need any changes on the side of the subscriber.<br>
 * Subscribers are totally agnostic about the event bus.
 * </p>
 *
 * <blockquote>
 *
 ...
 *
 * <p>
 * Lambda expressions might be used directly as {@link Consumer}s.
 * </p>
 *
 * <blockquote>
 *
 ...
 * <h2>Behavior</h2>
 * <p>
 * Publishing and dispatching is supposed to work in multi-threaded
 * applications.<br>
 * Wiring of publishers and subscribers is expected to be done a start time but
 * might also work dynamically at runtime.
 * </p>
 * <p>
...
public interface LambdaBus
     ...
     * registered for the {@link Class} of the event, subscribers will be searched
     * by the directly implemented interfaces of the event. Dispatching will occur
     ...
     * registered for the class of the event, subscribers will be searched by the
     ...
     *
     * <p>
     * By convention the {@link Class} of an event is evaluated first for subscriber
     * lookup.<br>
     * If there are is no matching subscriber for the {@code Class} of an event the
     * directly implemented interfaces of the event will be evaluated in order of
     * appearance.<br>
     ...
     * @param eventHandler
     *            non-{@code null} {@link Consumer} which should get events of the
     *            given class
     * @return non-{@code null} {@link Subscription} which will unsubscribe the
     *         {@code eventHandler} from the bus on closure
     * @throws NullPointerException
     ...
     * <p>
     * Posting events to the bus or subscribing to the bus after {@code close()} is
     * expected to throw an {@link IllegalStateException}.
     * </p>
     *
     * @throws IllegalStateException
     *             if the bus has been closed already
