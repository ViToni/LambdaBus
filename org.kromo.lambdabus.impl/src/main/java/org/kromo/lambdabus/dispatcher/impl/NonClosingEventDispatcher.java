...
public class NonClosingEventDispatcher
    ...
    public void close() {
        if (isClosed.compareAndSet(false, true)) {
            logger.debug("Called close() - call will not be delegated to: {}",
     ...
     *
     * @return returns {@code true} if {@link #close()} has been called yet,
    ...
    public boolean isClosed() {
        ...
        if (eventDispatcher.isClosed() && isClosed.compareAndSet(false, true)) {
            logger.debug("Closing because of closed internal EventDispatcher: {}",
                    eventDispatcher.getClass()
                            .getSimpleName());
        }
